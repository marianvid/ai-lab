...
class VoxCPMDemo:
    ...
    def generate_tts_audio(
        ...
        # The installed VoxCPM 2.0.3 accepts no seed argument. Seed PyTorch
        # directly for that API version while preserving newer upstream APIs.
        if "seed" not in inspect.signature(current_model._generate).parameters:
...
def create_demo_interface(demo: VoxCPMDemo):
    ...
    with gr.Blocks() as interface:
        gr.HTML(
            ...
            f'<img src="/gradio_api/file={Path(__file__).parent / "assets" / "voxcpm_logo.png"}" alt="VoxCPM Logo">'
            "</div>"
        )
            ...
            with gr.Column():
                ...
                with gr.Accordion(I18N("advanced_settings_title"), open=False):
                    DoDenoisePromptAudio = gr.Checkbox(
                        ...
                        elem_classes=["switch-toggle"],
                        interactive=bool(getattr(demo.voxcpm_model, "denoiser", None)),
                        info=(I18N("ref_denoise_info")
                              if getattr(demo.voxcpm_model, "denoiser", None)
                              else "Unavailable: this AI-Lab runtime has no ZipEnhancer checkpoint."),
                    )
